...
async function initConfig() {
  ...
  if (storageType !== 'localstorage') {
    ...
    try {
      ...
      if (adminConfig) {
        ...
        adminConfig = {
          ...
          SiteConfig: {
            SiteName: process.env.NEXT_PUBLIC_SITE_NAME || 'DreamTV',
            Announcement:
    ...
    cachedConfig = {
      ConfigFile: JSON.stringify(fileConfig),
      SiteConfig: {
        SiteName: process.env.SITE_NAME || 'DreamTV',
        Announcement:
          process.env.ANNOUNCEMENT ||
          '本网站仅提供影视信息搜索服务，所有内容均来自第三方网站。本站不存储任何视频资源，不对任何内容的准确性、合法性、完整性负责。',
        SearchDownstreamMaxPage:
...
export async function getConfig(): Promise<AdminConfig> {
  ...
  if (adminConfig) {
    ...
    adminConfig.SiteConfig.SiteName =
      adminConfig.SiteConfig.SiteName || process.env.NEXT_PUBLIC_SITE_NAME || 'DreamTV';
